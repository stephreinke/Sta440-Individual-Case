...
test_obese <- tidy(KRmodcomp(fit_REE, fit_no_obese))
test_vent <- tidy(KRmodcomp(fit_REE, fit_no_vent))
test_ICU <- tidy(KRmodcomp(fit_REE, fit_no_icu))
```
...
test_obesekg <- tidy(KRmodcomp(fit_REEperKG, fit_no_obesekg))
test_ventkg <- tidy(KRmodcomp(fit_REEperKG, fit_no_ventkg))
test_ICUkg <- tidy(KRmodcomp(fit_REEperKG, fit_no_icukg))
```
...
```{r}
#| fig-cap: Residual plot for REE model shows okay scatter but coning shaped variance
plot(fitted(fit_REE), resid(fit_REE))
abline(h = 0, col = "red", lwd = 2, lty = 2)
```

```{r}
#| fig-cap: QQ plot for REE model shows slight devation from diagnol at center and tails
qqnorm(resid(fit_REE)); qqline(resid(fit_REE))
```

```{r}
#| fig-cap: QQ plot by patient for REE model shows deviation from normality
qqnorm(ranef(fit_REE)$ID[,1])
```

```{r}
#| fig-cap: Residual plot for REE KG model shows okay scatter but denser at lower fitted values
plot(fitted(fit_REEkg), resid(fit_REEkg))
abline(h = 0, col = "red", lwd = 2, lty = 2)
```

```{r}
#| fig-cap: QQ plot for REE KG model shows slight devation from diagnol at tails
qqnorm(resid(fit_REEkg)); qqline(resid(fit_REEkg))
```

```{r}
#| fig-cap: QQ plot by patient for REE KG model shows deviation from normality
qqnorm(ranef(fit_REEkg)$ID[,1])
```

```{r}
results <- bind_rows(
  mutate(test_obese, predictor = "Obese"),
  mutate(test_vent, predictor = "Ventilator"),
  mutate(test_ICU,  predictor = "ICUorSDU")
)

knitr::kable(results, digits = 4, caption = "Kenward–Roger Tests for Fixed Effects for REE Model")
```
```{r}
results <- bind_rows(
  mutate(test_obesekg, predictor = "Obese"),
  mutate(test_ventkg, predictor = "Ventilator"),
  mutate(test_ICUkg,  predictor = "ICUorSDU")
)

knitr::kable(results, digits = 4, caption = "Kenward–Roger Tests for Fixed Effects for REE per KG Model")
```
